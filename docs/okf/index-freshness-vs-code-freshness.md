...
1. Reindex via the **CLI** to isolate the ingest from the server:
   `npm run index` (which is `tsx src/index.ts index`, `package.json:22`).
...
`"build": "tsc"` (`package.json:20`). Even after rebuild, a *running* server
...
- If the client runs `npm run mcp`, that is `tsx src/mcp-server.ts`
  (`package.json:25`) — it transpiles **source** on each spawn. No `npm run
...
- On the `dist` path, if behavior did not change, confirm you actually rebuilt:
  ...
  `package.json:29`). A stale `dist` is the classic "I edited src and nothing
  happened" trap.
...
`loadConfig()` reads `process.env.ORACLE_*` (`src/config.ts:81-115`), and
...
DISCREPANCIES: None. Every claim in the doc was verified at source and the two prose assertions in `docs/architecture.md:46-49` (WAL cross-process reads; running server sees writes on next query without restart) are backed by the code (`journal_mode = WAL` at `sqlite-store.ts:178`; fresh per-statement reads with no JS result cache at `sqlite-store.ts:398/436/459`). The only correctness nuance I surfaced rather than a contradiction: (1) the `writeEpoch` heartbeat exists in the schema and is bumped on writes but is not consumed by any reader in `src/` to invalidate a cache, so it is not the mechanism behind DATA freshness; and (2) `npm run mcp`/`dev`/`index` run via `tsx` against `src/` (`package.json:21-26`), so the "compiled `dist` artifact" staleness only applies when the MCP client launches the installed `bin` (`dist/index.js`, `package.json:6-8`) — the doc separates these two launch paths explicitly.
