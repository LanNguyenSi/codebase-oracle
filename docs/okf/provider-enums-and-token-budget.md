...
timestamp: 2026-09-26T04:44:36Z
sources:
  - src/config.ts
...
- Not covered here (out of this doc's stated scope): every LLM constructor also now sets a shared request timeout and `maxRetries: 0` from `config.llmTimeoutMs` (`ORACLE_LLM_TIMEOUT_MS`, task `844aac2c`), see `../configuration.md` for that env var and its default's reasoning.
