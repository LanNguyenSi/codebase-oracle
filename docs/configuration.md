...
| `ORACLE_LLM_TIMEOUT_MS` | No | `120000` | Request timeout in milliseconds, applied both to every LLM constructor (Anthropic, OpenAI, and the `openai-compatible`/`ollama` lane) and as the overall deadline wrapped around the whole non-streaming `chain.invoke()` call, paired with a fixed `maxRetries: 0` so the answer LLM call never waits past this bound before falling back to raw retrieved context (`degraded: true` in `--json` output). The bound covers the entire answer (model load, prompt evaluation, and the full response), not just time to first byte, so it also catches an endpoint that sends response headers and then stalls the body. Retrieval (question embedding and similarity search) runs before the LLM call and is not covered by this bound. Default reasoning: a cold local Ollama model can take well over a minute to load into memory and evaluate a long prompt on CPU before finishing a response, so a short default here would misclassify a slow-but-healthy local endpoint as unreachable; if a slow local setup still trips this default, raise it. Same positive-integer / fail-loud-on-invalid-value contract as `ORACLE_MAX_FILE_SIZE`, plus a `2147483647` ceiling (Node's `setTimeout` delay range): unset (or the empty string) falls back to the default, a set-but-invalid value (`abc`, `0`, `-5`, or one above the ceiling) fails config loading loudly. |
...
`npm run index` scans all git repos under `ORACLE_SCAN_ROOT`. By default it loads JS/TS sources (`.ts`, `.tsx`, `.js`, `.jsx`, `.vue`), docs (`.md`), sibling languages (`.py`, `.php`, `.go`, `.rs`, `.java`), config/infra (`.yaml`, `.yml`, `.toml`, `.sql`, `.prisma`, `.sh`), and the `package.json` / `tsconfig.json` manifests. Files over `ORACLE_MAX_FILE_SIZE` bytes (default 500 KB) are skipped, except `.md` files, which get the larger `ORACLE_MAX_TEXT_FILE_SIZE` ceiling (default 2 MB) instead. Empty files are silently skipped (nothing to index); files over the applicable size limit and files that fail to read (permission errors, binary decode failures) are each reported on stderr at index time, one line per file naming the path, the reason, and the specific env var whose limit was exceeded, followed by a one-line total — never a silent drop. `npm run watch` reports the same way through its console logging when a changed file trips the limit, applies the same per-type ceiling, but only on the console: it never updates the persisted per-repo skip count described below (see "Keeping the index fresh"; that count reflects only the last full `npm run index` / `oracle_reindex` run). A repo's skipped-file count from the last full index run is visible per repo via `oracle_list_repos` / `list-repos` whenever anything was skipped, rendered as a total broken down by reason (e.g. `2 too large, 1 read error`) followed by up to `SKIP_EXAMPLES_LIMIT` (5) example paths, e.g. `; 3 file(s) skipped in the last index run (2 too large, 1 read error; e.g. harness/CHANGELOG.md, ...)`. A repo whose files were skipped in their entirety (zero indexed chunks) still appears in the listing with this count, rather than being omitted for having nothing else to show.
