...
| `ORACLE_LLM_TIMEOUT_MS` | No | `60000` | Request timeout in milliseconds, applied to every LLM constructor (Anthropic, OpenAI, and the `openai-compatible`/`ollama` lane), paired with a fixed `maxRetries: 0` so a query never waits past this bound before falling back to raw retrieved context (`degraded: true` in `--json` output). Default reasoning: a cold local Ollama model can take tens of seconds to load into memory before its first token, so a short default here would misclassify a slow-but-healthy local endpoint as unreachable; 60 s leaves headroom for that cold start while still bounding a genuinely stuck request. Same positive-integer / fail-loud-on-invalid-value contract as `ORACLE_MAX_FILE_SIZE`: unset (or the empty string) falls back to the default, a set-but-invalid value (`abc`, `0`, `-5`) fails config loading loudly. |
