...
- **Byte-identical guarantee:** for a corpus where no retrieved chunk carries `fmSources` (or when expansion is disabled), the `oracle_search` result list and its rendered output are byte-identical to v0.8.0. The `expandedFrom` marker is transient — it lives only on the returned Document and is never persisted.
