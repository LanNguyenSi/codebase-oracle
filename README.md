...
- **Failure**: the document is always `{"ok": false, "error": {"message": "..."}}` (or, for `expand`, the pre-existing `{"ok": false, "reason": "...", "message": "..."}` shape) and the process exits nonzero. This was already true in 0.12.0 for argument/unknown-option errors on all four commands.
