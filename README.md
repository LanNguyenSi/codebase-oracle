...
Every answer LLM call (Anthropic, OpenAI, and the `openai-compatible`/`ollama` lane) is bounded by `ORACLE_LLM_TIMEOUT_MS` (default 120000ms, retries disabled): an unreachable or unresponsive endpoint falls back to raw retrieved context within that bound instead of hanging. The bound covers the whole non-streaming answer — model load, prompt evaluation, and the full response, not just time to first byte — so it also catches an endpoint that sends response headers and then stalls. The default is chosen with a cold local Ollama model in mind, since loading a multi-GB model into memory plus evaluating a long prompt on CPU can take well over a minute on a cold start; a shorter default would misclassify that as unreachable. If a slow local setup still trips this default, raise `ORACLE_LLM_TIMEOUT_MS`. See [docs/configuration.md](docs/configuration.md) for the full env-var reference.
